...
- BingX API 클라이언트 (bingx_client.py) 생성
  - HMAC SHA256 인증 시스템
  - 주문 실행, 잔고 조회, 시장 데이터 API
  - 기존 CCXT 구조와 호환되는 인터페이스
- 거래소 추상화 계층 구현
  - 바이낸스와 BingX 공통 인터페이스
  - realtime_trading_engine.py에 BingX 지원 추가

### Phase 2: 데모 트레이딩 시스템 (3-4일)

- 데모 트레이딩 시뮬레이터 (demo_trading.py) 구현
  - 가상 잔고 및 포지션 관리
  - 실거래와 동일한 API 인터페이스
  - 거래 기록 및 성과 추적
- 모드 전환 시스템
